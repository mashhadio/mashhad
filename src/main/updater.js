...
// Where to fetch update metadata from.
//
// NOT from package.json when packaged: electron-builder strips the `build` key out of
// the package.json it writes into app.asar (it is build-time config), so
// `require('../../package.json').build` is undefined in every shipped build and the
// destructure below throws. That returned null, `assetUrl` returned null, and
// checkMacUpdate bailed at `if (!ymlUrl) return` — silently. The macOS banner therefore
// never appeared in ANY release; it only worked when running from source, where
// package.json is the real one. (Windows/Linux were unaffected: electron-updater reads
// app-update.yml, not package.json.)
//
// app-update.yml is generated by electron-builder from that same `build.publish` block
// and shipped next to the asar, so it stays a single source of truth — it is what
...
function publishTarget() {
  if (app.isPackaged) {
    try {
      ...
      if (provider) return { provider, owner: field('owner'), repo: field('repo'), url: field('url') };
      console.warn('[update] app-update.yml has no provider');
    } catch (err) {
      console.warn('[update] could not read app-update.yml:', err && err.message);
    }
    return null;
  }
  // Unpackaged (npm start): app-update.yml does not exist, so use the real package.json.
  try {
